...
export default function MovieHoverCard({ movie }: CardProps) {
    ...
    debounceTimerRef.current = setTimeout(async () => {
      if (!videoKey) {
        try {
          ...
          // FIXED: Corrected endpoints to include /3/movie/ and explicit URL paths
          const res = await fetch(`https://api.tmdb.org/3/movie/${movie.id}/videos?api_key=${apiKey}`);
  ...
  // FIXED: Prepend the verified standard TMDB image base configuration and fallback sizing
  const posterUrl = movie.poster_path
    ? `https://image.tmdb.org/t/p/w500${movie.poster_path.trim()}`
    : "https://unsplash.com"; // Valid placeholder image file path
    ...
    >
      ...
      {isHovered && videoKey && (
        <div className="absolute inset-0 w-full h-full bg-black animate-in fade-in duration-200">
          <iframe
            // FIXED: Standardized embedding URL format path for YouTube
